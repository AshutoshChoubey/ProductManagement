...
// console.log("suppler",out);
  ...
  addStock()
  {
    ...
    // console.log(this.data);
     ...
     //  console.log(stock.supplier);
      ...
      if( stock.supplier=='undefined' || stock.supplier=='undefined' || stock.supplier==''|| typeof(stock.supplier) == "undefined" ){

        Swal.fire('Something is Missing', 'Please select supplier', 'error');
        this.submited=0;
        return 0;
        
      }

      if( stock.product_price=='undefined' || stock.product_price=='undefined' || stock.product_price==''|| typeof(stock.product_price) == "undefined" ){

        Swal.fire('Something is Missing', 'Please providde prodct price', 'error');
        this.submited=0;
        return 0;
        
      }

       if(stock.supplier==''|| typeof(stock.supplier) == "undefined"  || typeof(stock.purchaseInvoie) == "undefined" || typeof(stock.quantity) == "undefined" || stock.quantity=='' || stock.purchaseInvoie=='' || stock.entry_date=='' || stock.product_price =='')
        {
         this.overConsume=1;
        }
  ...
  handleAddStockRequest(data)
  {
    ...
    if(data==1)
    {
        Swal.fire('Good job', 'stock added', 'success');
